const Card = ({ pokemon, size }: { pokemon: any, size: 'small' | 'full' }) => {
    ...
    const color = typeColorMap[pokemonType] || 'gray';
  
    if (pokemon.sprites) {
      return (
        <div className={`flex items-center flex-col bg-gradient-to-br from-${color}-300 via-${color}-400 to-${color}-500 justify-start border-zinc-300 border border-10 rounded-xl shadow-md ${size === 'small' ? 'w-[250px] h-[350px]' : 'w-[350px] h-[500px]'} text-zinc-900`}>
          {/* header */}
          ...
          <div className="flex items-center flex-col justify-center w-11/12">
            <div className={`w-full ${size === "small" ? 'max-h-40' : 'max-h-50'} border border-3 border-zinc-200 rounded-tl rounded-tr flex items-center justify-center bg-${color}-500`}>
    ...
    return (
      <div className={`flex items-center flex-col bg-gradient-to-br from-${color}-300 via-${color}-400 to-${color}-500 justify-start border-zinc-300 border border-10 rounded-xl shadow-md ${size === 'small' ? 'w-[250px] h-[350px]' : 'w-[350px] h-[500px]'} text-zinc-900`}>
        ...
        <div className="flex items-center flex-col justify-center w-11/12">
          ...
          <div className="flex items-center justify-center w-full border border-3 border-zinc-200 rounded-bl rounded-br bg-zinc-100 border-t-0">
            <p className="text-[8px] font-bold text-zinc-600">NO. {String(pokemon.id).padStart(4, "0")} {pokemon.types.slice(0, 1).toUpperCase() + pokemon.types.slice(1)} Pokemon HT: {
